...
public class RndInputLoader extends InputLoader implements JWATConstants{
	 ...
	 * @param prg
	 */
	public RndInputLoader(Parameter param, String fileName,
			VariableMapping[] map, ProgressShow prg) throws FileNotFoundException {
			...
			for(k=0;k<=maxInd;k++)
			{
				...
				if(i==param.getNumVar())
				{
					if(input.endOfInput()){
						...
						{
							if(sel[i]){
								if(map[i]==null) lineValue[j++]=Double.parseDouble(lineToken[i]);
								else{
	...
	private int generateRandom(int numTotal,int numToRead,boolean[] catchVal)
	{
		...
		dimArray=numTotal;
		for(i=0;i<dimArray;i++) rndArray[i]=i;
		
		for(i=0;i<numToRead;i++)
		{
			rndNum= (int) (mst.nextDouble()*dimArray);
			catchVal[rndArray[rndNum]]= true;
			if(max<rndArray[rndNum]) max=rndArray[rndNum];
