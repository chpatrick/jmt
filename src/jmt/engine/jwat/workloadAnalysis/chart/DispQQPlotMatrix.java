					...
					public void mouseClicked(MouseEvent e)
					{
						...
						if (e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 2)
						{
							...
							if(e.getX() < x && e.getY() < y && e.getX() / WIDTH_TOT != e.getY() / HEIGHT_TOT)
							{
								...
								f.setSize(400,400);
								f.setTitle("QQ-Plot of " + DispQQPlotMatrix.this.model.getMatrix().getVariables()[e.getX() / WIDTH_TOT].getName() + " and " +
										DispQQPlotMatrix.this.model.getMatrix().getVariables()[e.getY() / HEIGHT_TOT].getName());
								...
								for(int row=0;row<Elenco.length;row++)
								{
									for(int col=0;col<Elenco.length;col++)
									{
										...
										{											
											...
											for(int i = 1;i < 9; i++){
												g.drawLine(startx,starty + (i * (HEIGHT_GRAPH / 9)),startx+WIDTH_GRAPH,starty + (i * (HEIGHT_GRAPH / 9)));
												g.drawLine(startx + (i * (WIDTH_GRAPH/ 9)),starty,startx + (i * (WIDTH_GRAPH/ 9)),starty+HEIGHT_GRAPH);
											}
