		...
		if(first){
			...
			new TimeConsumingWorker(new ProgressMonitorShow(this,"Plotting observations ...",100)){
				public Object construct() {
					VectorGraphics g1 = VectorGraphics.create((Graphics2D)graph.getGraphics());
					...
					for(int i = 0;i < x.Size(); i++){
						try{
							...
							g1.fillOval((int)((x.getValue(xPos) - xMin) / (xMax - xMin) * (WIDTH-2)),(HEIGHT-2) - (int)((x.getValue(xPos,yVar) - yMin) / (yMax - yMin) * (HEIGHT-1)
								),pointSize,pointSize);
						}catch(Exception e){
							break;
						}
