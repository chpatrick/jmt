...
public class FuzzyOptPanel extends JPanel implements CommonConstants{
	...
	private final String fuzzylevelDescription = HTML_START + HTML_FONT_NORM +
	...
	private final String FKMEANS_OPTIONS_DESCRIPTION = HTML_START
		...
		+ "Specify the transformation to apply to<p>" +
		  "selected variables." +
		  HTML_FONT_NOR_END
		+ HTML_END;
	private final String FKMEANS_FLEVEL_DESCRIPTION = HTML_START
	+ HTML_FONT_TITLE + "Fuzziness Level" + HTML_FONT_TIT_END + HTML_FONT_NORM
	+ "Fuzziness level is the fuzzy exponent and<p>" +
	  "range from (2 to 100).It determines the<p>" +
	  "degree of fuzziness of the final solution" + HTML_FONT_NOR_END
	+ HTML_END;
	private final String FKMEANS_CLUSTER_OPT = HTML_START + HTML_FONT_NORM
	...
	private void initPanel(){
		...
		fuzzyPanel.setBorder(new TitledBorder(new TitledBorder(
				new EtchedBorder(EtchedBorder.LOWERED), "Fuzzy options")));
		...
		fuzzyLevel = new JSpinner(new SpinnerNumberModel(2,2,100,1));
	...
	private void setSolveAction(){
		...
		((JWatWizard)parent.getParentWizard()).setEnableButton("Solve",true);
		((JWatWizard)parent.getParentWizard()).setActionButton("Solve",new AbstractAction(){
			public void actionPerformed(ActionEvent e) {
				if(parent.getVarSelected().length >= 2){
					...
					MainFuzzyKMean eng = new MainFuzzyKMean(
							...
							((Integer)fuzzyLevel.getValue()).intValue(),
							trasf
					);
		...
		((JWatWizard)parent.getParentWizard()).setActionTool(new AbstractAction(){
			{
