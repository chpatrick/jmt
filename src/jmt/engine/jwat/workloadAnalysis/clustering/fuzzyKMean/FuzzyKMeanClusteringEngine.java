...
/** DA SISTEMARE **/		String[] clus_log; /*= new String[37];	// log dei valori del Cluster = new String[MAXCLU]*/
			...
			for(int kh=0;kh<m_nIter;kh++)
			{
				...
				for (int i=0; i<cltemp; i++) {		//inizializza/calcola mrow
					...
					for (int k=0; k<nObs; k++) {
						mrow[i][1] = mrow[i][1]+Math.pow(U[i][k],(double)m_nFLev);
				...
				for (int i=0; i<cltemp; i++) {        //calcola i centri
					for (int j=0; j<m_nNumNorm; j++) {
						...
						for (int k=0; k<nObs; k++) {
							...
							centers[i][j]=centers[i][j] + Math.pow(U[i][listOfVars[j].getObsID(k)-1], (double)m_nFLev)*listOfVars[j].getValue(k);
						}
						centers[i][j] =centers[i][j]/mrow[i][1];
					}
				}
				/*** OK ***/
				for (int k=0; k<nObs; k++)      //inizializza matrice singolarità
				...
				for (int i=0; i<cltemp; i++) {  //calcola distanza datapoint-centri
					for (int k=0; k<nObs; k++) {
						...
						for (int j =0; j<m_nNumNorm; j++) {
							...
							distance[i][obsSel[k].getID()-1] = distance[i][obsSel[k].getID()-1] + Math.pow(centers[i][j]-obsSel[k].getIndex(varS[j]),2.0);
						}
						distance[i][obsSel[k].getID()-1]=Math.sqrt(distance[i][obsSel[k].getID()-1]);
						if (distance[i][obsSel[k].getID()-1]==0) singularity[obsSel[k].getID()-1][0] = i;//era[1]
					}
				}
					...
					for (int k=0; k<nObs; k++) {
						if (singularity[obsSel[k].getID()-1][0]>=0) {//era[1]
							for (int j=0; j<cltemp; j++) {
								if (j==singularity[obsSel[k].getID()-1][0])//era[1]
							...
							for (int j=0; j<cltemp; j++) {
								newU[i][obsSel[k].getID()-1] = newU[i][obsSel[k].getID()-1] + Math.pow(distance[j][obsSel[k].getID()-1],(double)(-2/(m_nFLev-1)));
							}
							newU[i][obsSel[k].getID()-1] = newU[i][obsSel[k].getID()-1] *Math.pow(distance[i][obsSel[k].getID()-1], (double)(2/(m_nFLev-1)));
	...
	private void initMatrix(int pos,int row,int col){
		...
		switch(pos){
		case 0: U = new double[row][col];
			//m = U;
			//inizializzazione come suggerito da E.N.Magenis
			for (int i=0; i<row; i++){
				for (int j=0; j<col; j++){
					if ((i==j)||((i==row-1)&&(j>row-1))) U[i][j]=1;
					else U[i][j]=0;
				}
			}
			break;
		case 1: newU = new double[row][col];
			//m = newU;
			break;
		case 2: centers = new double[row][col];
			//m = centers;
			break;
		case 3: mrow = new double[row][col];
			//m = mrow;
			break;
		case 4: singularity = new double[row][col];
			//m = singularity;
			break;
		case 5: distance = new double[row][col];
			//m = distance;
			break;
		}
	}
