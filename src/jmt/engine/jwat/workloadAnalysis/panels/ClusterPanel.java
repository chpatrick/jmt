...
import jmt.engine.jwat.workloadAnalysis.clustering.fuzzyKMean.panels.FuzzyOptPanel;
import jmt.engine.jwat.workloadAnalysis.clustering.kMean.panels.KMeansOptPanel;
...
public class ClusterPanel extends WizardPanel implements CommonConstants{
	 
	private final String CLUSTERING_DESCRIPTION = HTML_START
		+ HTML_FONT_TITLE + "Clustering" + HTML_FONT_TIT_END + HTML_FONT_NORM
		+ "Select clustering algorithm, set options and press solve" + HTML_FONT_NOR_END
		+ HTML_END;
	...
	public ClusterPanel(WorkloadAnalysisWizard parent)
	{
		this.parent = parent;
		...
		public void statusEvent(EventStatus e) {
			...
			switch(e.getType()){
				case EventStatus.ABORT_EVENT:	abortEvent((EventFinishAbort)e);
					break;
				case EventStatus.DONE_EVENT:	finishedEvent((EventClusteringDone)e);
					break;
			}
		}
	...
	"<HTML>" +
	"In this panel you can choose which clustering algorithm apply to data.<br><br>" +
