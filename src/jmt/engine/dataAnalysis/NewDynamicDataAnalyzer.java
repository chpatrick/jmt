...
//      point.
        ...
        if ((prevprev < prev * (1-hEpsilon) && prev < mean * (1-hEpsilon)) ||
            (prevprev > prev * (1+hEpsilon) && prev > mean * (1+hEpsilon))) {
        ...
        if (hSamplesNum % checkSize == 0){
            ...
            for (int i=0; i<hSamplesNum - 2; i++)
                ...
                if (hMeans[i] >= lower && hMeans[i+1] <= upper ||
                   (hMeans[i] <= upper && hMeans[i+1] >= lower )) {
                    crosses++;
...
// --- Old Code by Granata ---------------------------------------------------------------------
    ...
    protected boolean HWtest() {
        ...
        if (precision > confInt / extMean && !disableStatisticStop) {
            ...
            if (batch == (numBatch - 1)) {
                ...
                int log2;
                log2 = 1 << ((int) Math.ceil((Math.log(Math.sqrt(nsamples)))
                        / Math.log(2)));
                if (log2 < 64)
                    log2 = 64;
                if (log2 > numBatch) {
                    ...
                    try {
                        System.arraycopy(batchMean, 0, tempBatch, 0,
                                batchMean.length);
                    } catch (Exception e) {
                        System.out.println(e);
                        System.out.println(batchMean.length + "  "
                                + tempBatch.length);
                    }

                    batchMean = tempBatch;
                    tempBatch = new double[numBatch];
                    System.arraycopy(weightBatchMean, 0, tempBatch, 0,
                            weightBatchMean.length);
                    ...
                    for (int i = 0; i < half; i++) {
                        batchMean[i] = (batchMean[2 * i]
                                * weightBatchMean[2 * i] + batchMean[2 * i + 1]
                                * weightBatchMean[2 * i + 1])
                                / (weightBatchMean[2 * i]
                                + weightBatchMean[2 * i + 1]);
                        weightBatchMean[i] = (weightBatchMean[2 * i]
                                + weightBatchMean[2 * i + 1]) / 2;
                    }
    ...
    /** calculates the variance through Heidelberger & Welch method, if the data
        ...
        public double calcVar(double[] data, int start, int n, int K, int polyOrder) {
            ...
            if (n < 4 * K)
                throw new Error("number of data must be at least 4 times K: "
                        + data.length);

            intData = Spectrum.periodogramOfReal(data, start, n);
            for (int i = 0; i < K; i++) {
                fn[i] = ((4 * ((double) i + 1)) - 1) / (2 * (double) n);
                J[i] = (Math.log((intData[((2 * (i + 1)) - 1)]
                        + intData[(2 * (i + 1))]) / 2)) + 0.270;
            }
            //polynomial fitting
        ...
        protected double calcExtimatorVar(int K, int polyOrder) {
            ...
            for (int i = 0; i < K; i++) {
                for (int j = 0; j <= polyOrder; j++)
                    X.set(i, j, (Math.pow((4 * ((double) i + 1) - 1)
                            / (2 * (double) K), j)));
            }
            Xt = X.transpose();
            square = Xt.times(X);
	...
	protected boolean deleteTransient() {
		...
		for (int i = data - 2; i > index; i--) {
			...
			sumSquare += miniW_elem * y * y - miniW_elem * miniW_elem * y
			        * y / sumW;
			oldMean = mean;
		}
        ...
        if (d - 1 <= nsamples / (batches.getBatchSize() * batches.getBatchSize())) {
			...
			numBatch = 1 << ((int) Math.ceil((Math.log(Math.sqrt(nsamples)))
			        / Math.log(2)));
			if (numBatch < 64)
				numBatch = 64;
