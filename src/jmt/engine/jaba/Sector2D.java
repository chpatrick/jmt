...
public class Sector2D {
    ...
    public Sector2D addCollinear(Sector2D ss,newPoint p3)
    {
        ...
        if (
            uti.Collinear(ss.getP1(),ss.getP2(),p3) &&
            p3.getX()>ss.getP1().getX() &&
            p3.getX()<ss.getP2().getX()    
           )
        vp.addElement(p3);
        return ss;
    }
    ...
    public Sector2D addCollinearLast(Sector2D ss,newPoint p3)
    {
        ...
        if((double)ss.getP1().getX()==p3.getX() && (double)ss.getP1().getY()!=p3.getY())
        {
            vp.addElement(p3);
        }
        return ss;
    ...
    public Sector2D addCollinearFirst(Sector2D ss,newPoint p3)
    {
        ...
        // Riga alternativa usata come prova
        if((double)ss.getP1().getY()==p3.getY() && (double)ss.getP1().getX()!=p3.getX())
        {
            vp.addElement(p3);
        }
        return ss;
    ...
    public int AssoStation2D(Vector points,newPoint p)
    {
        ...
        for (int i=0;i<points.size();i++)
        {
            ...
            if (
                    p.getX()==((newPoint)points.get(i)).getX() &&
                    p.getY()==((newPoint)points.get(i)).getY()
               )
            {
                stat = i;
            }
