...
public class Calc {
    ...
    public Vector Calc2D(Vector vertices2D){
        ...
        if (verticesnp.size()>1) verticesnp = util.RemoveCollinear(verticesnp);
            ...
            */
            ...
            for (int i=0;i<res.size();i++)  //per tutti i settori
            {
                for (int j=0;j<original2D.size();j++)   //per tutti i punti originali
                {
                    ...
                    if (i==0 && ((newPoint)((Sector2D)res.get(i)).getP1()).getX()!=0)
                    {
                    ...
                    if (i==res.size()-1 && ((newPoint)((Sector2D)res.get(i)).getP1()).getY()!=0)
                    {
...
//------------------------------ CASO IN CUI NON CI SONO FACCE ------------------------------------
        ...
        */
        ...
        for (int i = 0; i<sett1staz.size();i++){
            allres.addElement((Sector3D)sett1staz.get(i));
        }
