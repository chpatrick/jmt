...
public class FinalSect2D {
    ...
    public FinalSect2D(Sector2D sect,Vector stats, String[] classNames)
    {
        ...
        {
            if (
                    ((newPoint)((Station2D)stats.get(i)).getVert()).equals(sect.getP1()) ||
                    ((newPoint)((Station2D)stats.get(i)).getVert()).equals(sect.getP2())
            )
            {
        ...
        if (sect.hasCollinear())
        {
...

            {
                ...
                {
                    if (
                            ((newPoint)((Station2D)stats.get(i)).getVert()).equals((newPoint)coll.get(c))
                    )
                    {
                        stations.addElement(stats.get(i));
                    }
