...
import jmt.engine.random.MMPP2Par;
...
public class MMPP2Distr extends AbstractDistribution implements Distribution {
	...
	public double theorMean(Parameter p)
		...
		return (((MMPP2Par) p).getSigma0()+((MMPP2Par) p).getSigma1())/(((MMPP2Par) p).getLambda0()*((MMPP2Par) p).getSigma1()+((MMPP2Par) p).getLambda1()*((MMPP2Par) p).getSigma0());
	}
