...
public class StatiDrawer extends JPanel implements Notifier, Runnable {
	...
	private double pb[];
	private int queueMax, currentJob, lastJob, jobTmp;
	...
	private void animateTransition(int from, int to, Color c, int frame, int maxframe, Graphics2D g2d) {
		...
		g2d.setColor(c);
		double x, y, diameter;
		x = (2.0 * STATUS_RAD + ELEMS_GAP) * (from + to) + START_GAP + (STATUS_RAD / 2.0);
	...
	private int queueLenght() {
		...
		tmp = (int) ((panelW - START_GAP - END_GAP + (2 * STATUS_RAD + 2 * ELEMS_GAP)) / (2 * (2 * STATUS_RAD + ELEMS_GAP)));
