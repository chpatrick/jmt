...
public class MMNdLogic extends MM1Logic {
	...
	public double getStatusProbability(int status) throws NonErgodicException{		
		...
		if(status <= numberOfServer)
			...
			result = pzero() * Math.pow((double)numberOfServer,(double)numberOfServer) *  
				Math.pow(p(),(double)status) / factorial(numberOfServer);
		else
			result = 0.0;
		return result;
	}
	...
	private double v() throws NonErgodicException{// m to m / m! * ((p)to the m) * (1- (p to the k+1)) /(1-p) 
		double p = p();
		return  Math.pow(numberOfServer, numberOfServer) / factorial(numberOfServer) * (Math.pow(p, (double) numberOfServer)) * (1 - Math.pow(p, (double) (max + 1))) 
					/ (1 - p);
