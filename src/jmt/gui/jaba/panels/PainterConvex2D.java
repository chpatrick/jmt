...
public class PainterConvex2D {
	...
	public void drawSelectLine(Graphics2D g, DPoint p1,DPoint p2,Vector s3d,String[] classNames)
	{
		if((p1!=null)&&(p2!=null))
		{
    		...
    		g.drawLine((int)(p1.getX()*scale)+ tran_x,tran_y-(int)(p1.getY()*scale),(int)(p2.getX()*scale)+ tran_x,tran_y-(int)(p2.getY()*scale));
    		g.setStroke(oldStro);
    		...
    		for (int i=0;i<s3d.size();i++) {
                ...
                if(sect.countStation()<2){continue;}
                
                Station2D d1 =(Station2D)(sect.getstation()).get(0);
                Station2D d2 =(Station2D)(sect.getstation()).get(1);
                int d1x = (int)((newPoint)d1.getVert()).getX();
                int d1y = (int)((newPoint)d1.getVert()).getY();
                int d2x = (int)((newPoint)d2.getVert()).getX();
                int d2y = (int)((newPoint)d2.getVert()).getY();
	...
	public boolean update(int height,int width)
	{
		if((this.height!=height)||(this.width!=width)){
			...
			pointSize = (int)Math.min(height,width)/100;
    ...
    public void summary(Graphics2D g)
    {
		...
		g.fillOval((int)5+(x)-((int)(size/2)),(int)(y+(0.5)*fontSize)-((int)(Math.min(pointSize+1,9)/2)), Math.min(pointSize+1,9), Math.min(pointSize+1,9));
		g.setColor(Color.black);
		g.drawString("Potential", x+11+((int)(size/2)), y);
		g.drawString("Bottleneck", x+11+((int)(size/2)), y+fontSize);
		g.drawString("Stations", x+11+((int)(size/2)), y+2*fontSize);
		
		//Masked-off
		g.setColor(Color.blue);
		g.fillOval((int)5+(x)-((int)(size/2)),-3+y+(4)*fontSize-Math.min(pointSize+1,9), Math.min(pointSize+1,9), Math.min(pointSize+1,9));
		g.setColor(Color.black);
		g.drawString("Masked-off", x+11+((int)(size/2)), 3+y+(3)*fontSize);
		g.drawString("Stations", x+11+((int)(size/2)), 3+y+(4)*fontSize);
		
		//Dominated
		g.setColor(new Color(15,185,100));
		g.fillOval((int)5+(x)-((int)(size/2)), 6+y+(((6*fontSize)+(5*fontSize))/2)-Math.min(pointSize+1,9), Math.min(pointSize+1,9), Math.min(pointSize+1,9));
		g.setColor(Color.black);
		g.drawString("Dominated", x+11+((int)(size/2)), 6+y+(5)*fontSize);
		g.drawString("Stations", x+11+((int)(size/2)), 6+y+(6)*fontSize);
		
		//Masked off-Area
		g.drawString("Masked-off", x+11+((int)(size/2)), 2+y+(8)*fontSize);
		g.drawString("Area", x+11+((int)(size/2)), 2+y+(9)*fontSize);
		int xP1 = 4+x-((int)(size/2));
		int yP1 = 3+y+(8)*fontSize+((int)(size/2));
		int xP2 = 4+x+size;
		...
		//Dominated Area
		g.setColor(Color.black);
		g.drawString("Dominated", x+11+((int)(size/2)), 3+y+(10)*fontSize);
		g.drawString("Area", x+11+((int)(size/2)), 3+y+(11)*fontSize);
		xP1 = 4+x-((int)(size/2));
		yP1 = 4+y+(10)*fontSize+((int)(size/2));
		xP2 = 4+x+size;
    ...
    public void drawPoint(Graphics g, Vector points,Color c,int size){
    	...
    	for(int j = 0; j < points.size(); j++){
    		...
    		g.setColor(c);
    		g.fillOval((int)(p.getX()*scale)-((int)(size/2))+ tran_x,tran_y-(int)(p.getY()*scale)-((int)(size/2)), size, size);
    	}
    }
    ...
    * This function draw te area among the axis and the point of the convex hull
    ...
    public void pointLabel(Graphics2D g,Vector points)
    {
    	...
    	for(int i=0;i<points.size();i++)
    	{
    		...
    		{
    			g.drawString(p.getLabel()+" ("+format2Dec.format(p.getX())+", "+format2Dec.format(p.getY())+")", tran_x+(int)(p.getX()*scale)-15, tran_y-(int)(p.getY()*scale)-3-pointSize);	
    		}
    	}
    }
    ...
    public void drawDominantArrow(Graphics2D g,Vector dominant,Vector dominates)
    {
	    ...
	    public void drawMousePosition(Graphics2D g,Point dragPoint)
	    {
	    		...
	    		g.drawLine((int)(dragPoint.getX()), (int)tran_y-pointSize+1,(int)dragPoint.getX(), (int)tran_y);
	    } 	
