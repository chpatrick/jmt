...
//    	Verifico se la cella e' il source o il target.
    	...
    	for(int i=0; i<controlPoint;i++){
    		controlPoints[i]=(Point2D)viewtmp.getPoint(i);
...
//    	if inseriti per evitare eccezioni di tipo nullPointerException
    	...
    	for(int i=0; i<controlPoint;i++){
    		...
    		controlPoints[i]=(Point2D)edgeView.getPoint(i);
    		if((controlPoints[i]==null))
    			return false;
        	...
        	if(targetOfEdge1.equals(targetOfEdge2)){
        		...
        		for(int i=0;i<listEdgesIn.length;i++){
        			...
        			for(int x=0; x<controlPoint3;x++){
        	    		controlPoints3[x]=(Point2D)edgeView1.getPoint(x);
        			...
        			if(((controlPoints3[controlPoints3.length-2].getX()>controlPoints[controlPoints.length-1].getX())&&(controlPoints[controlPoints.length-2].getX()>controlPoints3[controlPoints3.length-2].getX()))||(controlPoints3[controlPoints3.length-2].getX()<controlPoints[controlPoints.length-1].getX())&&(controlPoints[controlPoints.length-2].getX()<controlPoints3[controlPoints3.length-2].getX())){
        		...
        		for(int i=0;i<listEdgesOut.length;i++){
        			...
        			if((controlPoints[x].getX()>controlPoints3[x+1].getX())&&(controlPoints[x+1].getX()<controlPoints3[x+1].getX())&& ((int)controlPoints3[x].getY()==(int)controlPoints[x].getY())){
        				boolean contiene=false;
        			...
        			}else if((controlPoints[x+1].getX()>controlPoints3[x+1].getX())&&(controlPoints[x].getX()<controlPoints3[x+1].getX())&&(controlPoints[x].getY()==controlPoints[x+1].getY())){
        				boolean contiene=false;
        				if(intersectionEdgePoint.contains(controlPoints3[x+1])){
        					contiene=true;
        				}
...
//        			}
    	...
    	for(int i=0; i<controlPoint2;i++){
    		controlPoints2[i]=(Point2D)otherEdgeView.getPoint(i);
    			...
    			if(i==0 && sharedTheSameSourceTarget){
		    		...
		    		for(int q=0;q<controlPoints2.length-1;q++){
		    			...
		    			if(!(q%2==0)){
		    				...
		    				if((tmpMin2.getY()<tmpMin.getY())&&(tmpMax2.getY()>tmpMin.getY())&&(tmpMin2.getX()>tmpMin.getX())&&(tmpMin2.getX()<tmpMax.getX())){
...
//			System.out.println("Dentro il for");
			...
			if(sourceOfEdge!=sourceOfEdge2 && boundsSource.getMaxX()<boundsTarget.getMinX()-5 && boundsSource2.getMaxX()<boundsTarget.getMinX()-5){
