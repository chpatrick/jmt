...
//            	ringOffset=-ringOffset;
            ...
            if(source == target) {
            	...
            	if((int)from.getY()==(int)to.getY() && ((boundFrom<boundTo && offset>0 && offsetTo2<0)||(boundFrom>boundTo && offset<0 && offsetTo2>0))){
            		list.add(from);
            		list.add(to);
            		return list;
            	}
                if((((boundTo-boundFrom)>offset*(1.1))&&(offset>0 && offsetTo2<0))||(((boundFrom-boundTo)>(-offset*(1.1)))&&(offset<0 && offsetTo2>0))){
...
//                ____________inizio
                		...
                		routed = new Point2D[2];
                		if((sourceBounds.getMaxX() < boundTo + offsetTo2*2 && boundFrom<boundTo && to.getY() < from.getY())||(from.getY()<targetBounds.getMaxY() && sourceBounds.getMaxY()>to.getY())||((from.getY()>=targetBounds.getMinY() && from.getY()<=targetBounds.getMaxY())&& (sourceBounds.getMinY()<=targetBounds.getMaxY()))||(targetBounds.getMaxY()>=from.getY() && targetBounds.getMaxY()<=sourceBounds.getMaxY())){
                			routed=new Point2D[4];
                			routed[0]=new Point2D.Double(boundFrom+offset,from.getY());
                		...
                		int minX=Math.min((int)(boundFrom+offset-2), (int)(boundTo+(offsetTo2)-6));
                		if((int)minX==(int)(boundTo+(offsetTo2)-6))
                			minX=minX+4;		
                		minX=minX+2;
...
//                	int minY=0;
                	...
                	if((boundFrom>boundTo)&&((targetBounds.getMaxY()>from.getY() && targetBounds.getMinY()<from.getY())||(sourceBounds.getMaxY()<=targetBounds.getMaxY())&&((sourceBounds.getMaxY()>=to.getY()))||((sourceBounds.getMinY()>=targetBounds.getMinY())&&(sourceBounds.getMinY()<=to.getY())))){
                	routed=new Point2D[4];
                	routed[0]=new Point2D.Double(boundFrom+offset,from.getY());
                	routed[3]=new Point2D.Double(boundTo+offsetTo2, to.getY());
                	...
                	}else if((boundFrom<=boundTo)&&((targetBounds.getMaxY()>from.getY() && targetBounds.getMinY()<from.getY()) ||(from.getY() < to.getY() && boundFrom+offset*2 < targetBounds.getMinX()))){
...
//                    	}
                	...
                	int maxX=Math.max((int)(boundFrom+offset+2), (int)(boundTo+(offsetTo2)+6));

            		if((int)(boundTo+(offsetTo2)+6)==(int)maxX)
            			maxX=maxX-4;
            		maxX=maxX-2;
...
//                            routed[3].getY() + targetBounds.getBounds().getHeight()+8);
                    double maxY = Math.max(sourceBounds.getMaxY() + 20,
                            targetBounds.getMaxY() + 26);
                    routed[1] = new Point2D.Double(routed[0].getX(), maxY);
                    routed[2] = new Point2D.Double(routed[3].getX(), maxY);
