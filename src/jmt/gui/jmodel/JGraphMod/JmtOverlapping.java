...
//					
					...
					if(upperSideIntersaction && lowerSideIntersaction){
						...
						if((int)tmp.getX()<valoreIntermedio){
							Point newPosition= findFreePosition((JmtEdge)listEdges[i],(JmtCell)cellsfinal[j],cellBound, tmp,false,false,true,false);
							cellBounds.setLocation(newPosition);
							GraphConstants.setBounds(((JmtCell)cellsfinal[j]).getAttributes(), cellBounds);
						}else{
							Point newPosition= findFreePosition((JmtEdge)listEdges[i],(JmtCell)cellsfinal[j],cellBound, tmp,false,false,false,true);
							cellBounds.setLocation(newPosition);
							GraphConstants.setBounds(((JmtCell)cellsfinal[j]).getAttributes(), cellBounds);
						}
					}
...
//							System.out.println("LA MANDO GIU- "+(JmtCell)cellsfinal[j]+", valore intermedio:"+valoreIntermedio+", "+(int)tmp.getY());
							Point newPosition= findFreePosition((JmtEdge)listEdges[i],(JmtCell)cellsfinal[j],cellBound, tmp,false, true, false, false);
							cellBounds.setLocation(newPosition);
		        			GraphConstants.setBounds(((JmtCell)cellsfinal[j]).getAttributes(), cellBounds);
						}
						else{
//							System.out.println((JmtCell)cellsfinal[j]+", valore intermedio:"+valoreIntermedio+", "+(int)tmp.getY());
							Point newPosition= findFreePosition((JmtEdge)listEdges[i],(JmtCell)cellsfinal[j],cellBound, tmp,true,false,false,false);
							cellBounds.setLocation(newPosition);
		        			GraphConstants.setBounds(((JmtCell)cellsfinal[j]).getAttributes(), cellBounds);
						}
					}
...
//			System.out.println("Vertici da sistemare: "+vertexToMove.size());
	...
	public Point findFreePosition(JmtEdge edge,JmtCell vertex,Rectangle2D  cellBound,Point2D tmp,  boolean up,boolean down, boolean right,boolean left){
			...
			Rectangle newRettDown=new Rectangle((int)vertexMinX,(int)((int) (tmp.getY()+5)),(int)vertexWidth,(int)vertexHeight);
			...
			Rectangle newRettUp=new Rectangle((int)vertexMinX,(int)pointYToMove2-10,(int)vertexWidth,(int)vertexHeight);
			...
			Rectangle newRettLeft=new Rectangle((int)point2XToMove3,(int)vertexMinY,(int)vertexWidth,(int)vertexHeight);
...
//				int pointYToMove=(int) (tmp.getY()+(tmp.getY()-vertexMinY))+5;
				...
				Rectangle newRett=new Rectangle((int)vertexMinX,(int)pointYToMove,(int)vertexWidth,(int)vertexHeight);
				...
				for(int x=0; x<celletmp.length;x++){
					if(!(celletmp[x]==vertex) && !((vertex.getChildren()).contains(celletmp[x])) && !(celletmp[x]==edge)  ){
						if(celletmp[x] instanceof JmtEdge){
							...
							JmtEdgeView viewtmp=(JmtEdgeView) (mediator.getGraph().getGraphLayoutCache()).getMapping(((JmtEdge)(celletmp[x])), false);
							if(((handledEdges.size()==0) ||(handledEdges.size()>0 && !handledEdges.contains(celletmp[x])))&&((JmtEdge)celletmp[x]).intersects(viewtmp, newRett) ){
				...
				for(int x=0; x<celletmp3.length;x++){
					if(!(celletmp3[x]==vertex) && !((vertex.getChildren()).contains(celletmp3[x])) && !(celletmp3[x]==edge)  ){
						if(celletmp3[x] instanceof JmtEdge){
							JmtEdgeView viewtmp=(JmtEdgeView) (mediator.getGraph().getGraphLayoutCache()).getMapping(((JmtEdge)(celletmp3[x])), false);
							if(((handledEdges.size()==0) ||(handledEdges.size()>0 && !handledEdges.contains(celletmp3[x])))&&((JmtEdge)celletmp3[x]).intersects(viewtmp, newRettUp)){
...
//				System.out.println("Intersezioni di sotto: "+celle.size()+"Intersezioni di sopra:"+celle2.size());
				...
				while(celle.size()>0){
					...
					Rectangle new2rett=new Rectangle((int)vertexMinX,(int)pointYToMove,(int)vertexWidth,(int)vertexHeight);
					...
					for(int x=0; x<celletmp2.length;x++){
						if(!(celletmp2[x]==vertex) && !((vertex.getChildren()).contains(celletmp[x])) && !(celletmp2[x]==edge)  ){
							if(celletmp2[x] instanceof JmtEdge){
								JmtEdgeView viewtmp=(JmtEdgeView) (mediator.getGraph().getGraphLayoutCache()).getMapping(((JmtEdge)(celletmp2[x])), false);
			...
			//Begin IF to move the vertex up
				...
				for(int x=0; x<celletmp3.length;x++){
					if(!(celletmp3[x]==vertex) && !((vertex.getChildren()).contains(celletmp3[x])) && !(celletmp3[x]==edge)  ){
						if(celletmp3[x] instanceof JmtEdge){
							JmtEdgeView viewtmp=(JmtEdgeView) (mediator.getGraph().getGraphLayoutCache()).getMapping(((JmtEdge)(celletmp3[x])), false);
							if(((handledEdges.size()==0) ||(handledEdges.size()>0 && !handledEdges.contains(celletmp3[x])))&&((JmtEdge)celletmp3[x]).intersects(viewtmp, newRettDown)){
								celle2.add(celletmp3[x]);
							}
						}else{
						celle2.add(celletmp3[x]);
			...
			//BEGIN IF to move the vertex RIGHT
				...
				Rectangle newRett=new Rectangle((int)pointXToMove,(int)vertexMinY,(int)vertexWidth,(int)vertexHeight);
				Object[] celletmp=(mediator.getGraph()).getDescendants(mediator.getGraph().getRoots(newRett));
				Vector celle=new Vector();
				...
				for(int x=0; x<celletmp3.length;x++){
					...
					if(!(celletmp3[x]==vertex) && !((vertex.getChildren()).contains(celletmp3[x])) && !(celletmp3[x]==edge)  ){
						if(celletmp3[x] instanceof JmtEdge){
							JmtEdgeView viewtmp=(JmtEdgeView) (mediator.getGraph().getGraphLayoutCache()).getMapping(((JmtEdge)(celletmp3[x])), false);
							if(((handledEdges.size()==0) ||(handledEdges.size()>0 && !handledEdges.contains(celletmp3[x])))&&((JmtEdge)celletmp3[x]).intersects(viewtmp, newRettLeft)){
				...
				pointBack= new Point((int) pointXToMove,(int)vertexMinY);
			...
			//BEGIN IF to move the vertex LEFT
				...
				Rectangle newRett=new Rectangle((int)point2XToMove,(int)vertexMinY,(int)vertexWidth,(int)vertexHeight);
				Object[] celletmp=(mediator.getGraph()).getDescendants(mediator.getGraph().getRoots(newRett));
				Vector celle=new Vector();
				for(int x=0; x<celletmp.length;x++){
				...
				for(int x=0; x<celletmp4.length;x++){
					if(!(celletmp4[x]==vertex) && !((vertex.getChildren()).contains(celletmp4[x])) && !(celletmp4[x]==edge)  ){
							if(celletmp4[x] instanceof JmtEdge){
								JmtEdgeView viewtmp=(JmtEdgeView) (mediator.getGraph().getGraphLayoutCache()).getMapping(((JmtEdge)(celletmp4[x])), false);
								if(((handledEdges.size()==0) ||(handledEdges.size()>0 && !handledEdges.contains(celletmp4[x])))&&((JmtEdge)celletmp4[x]).intersects(viewtmp, newRettRight)){
				...
				pointBack= new Point((int) point2XToMove,(int)vertexMinY);
