    ...
    private void select(int i) {
        ...
        if (model.getWhatIfType().equals(ExactModel.WHAT_IF_ARRIVAL)) {
            if (model.getWhatIfClass() < 0)
                description.setText("Arrival rates \u03bbi of each open class ("
                        + doubleFormatter.format(value * 100) + "% of initial value)");
            else
                description.setText("Arrival rate for class "+ model.getClassNames()[model.getWhatIfClass()]
                        + " \u03bbi = "+ doubleFormatter.format(value) + " [job/s]");
        }
        else if (model.getWhatIfType().equals(ExactModel.WHAT_IF_CUSTOMERS)) {
            if (model.getWhatIfClass() < 0)
                description.setText("Number of customers Ni of each closed class ("+
                        doubleFormatter.format(value * 100) + "% of initial value)");
            else
                description.setText("Number of customers for "+ model.getClassNames()[model.getWhatIfClass()]+
                        " Ni = "+intFormatter.format(value));
        }
        else if (model.getWhatIfType().equals(ExactModel.WHAT_IF_DEMANDS)) {
            if (model.getWhatIfClass() < 0)
                description.setText("Service demands Di for all classes ("+
                        doubleFormatter.format(value * 100) + "% of initial value) at "
                        +model.getStationNames()[model.getWhatIfStation()]);
            else
                description.setText("Service demand for "+
                        model.getClassNames()[model.getWhatIfClass()]+
                        " at "+ model.getStationNames()[model.getWhatIfStation()]+
                        " Di = "+doubleFormatter.format(value)+" [s]");
        }
        else if (model.getWhatIfType().equals(ExactModel.WHAT_IF_MIX)) {
            description.setText("Population mix \u03b2i = " + doubleFormatter.format(value) +
                    " for "+model.getClassNames()[model.getWhatIfClass()]);
        }
    }
