...
public final class UtilizationPanel extends SolutionPanel{
	...
	private class UTableModel extends ExactTableModel {
		...
		protected Object getRowName(int rowIndex) {
            if (rowIndex == 0) return "<html>-</html>";
		...
		protected Object getValueAtImpl(int rowIndex, int columnIndex) {
            ...
            */
            //NEW Dall'Orso
            if(columnIndex>0 && rowIndex>0)d = util[rowIndex-1][columnIndex-1][iteration];
            else if(columnIndex==0 && rowIndex>0)d = stationAggs[rowIndex-1][iteration];
            else if(columnIndex>0 && rowIndex==0)d = classAggs[columnIndex-1][iteration];
            else d = globalAgg[iteration];
            //END
			if (d < 0) return "-"; //causes the renderer to display a gray cell
			return new Double(d);
		}
