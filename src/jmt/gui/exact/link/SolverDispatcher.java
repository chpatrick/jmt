	...
	private void solveMulti(ExactModel model, int iteration) throws SolverException, InputDataException {
		...
		if (model.isOpen()) {
			...
			if (model.isClosed()) {
				...
				try {		
					SolverMultiClosed closedsolver;
					switch(model.getPreferredAlg())
					{
					case ExactModel.PREFALG_AMVA_BS:
					{
						if (!model.isLd())
							closedsolver = new SolverMultiClosedAMVABS(classes, stations);
						else
							closedsolver = new SolverMultiClosedMVA(classes, stations);
						break;
					}
					case ExactModel.PREFALG_STANDARD:
					case ExactModel.PREFALG_MVA:
					default:
					{
						closedsolver = new SolverMultiClosedMVA(classes, stations);
						break;
					...
					if (!closedsolver.input(stationNames, stationTypes, serviceTimes, visits,classPop)) fail("Error initializing SolverMultiClosed", null);
					solver = closedsolver;
				} catch (SolverException se) {
					throw se;
				} catch (Exception e) {
