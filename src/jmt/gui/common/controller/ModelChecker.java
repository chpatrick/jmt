...
public class ModelChecker implements CommonConstants {
    ...
    */
    ...
    private Vector exploreForAllForwardStationAreSink(Object classKey, Object startingStationKey, Vector alreadyVisited) {
        ...
        if (forwardStations.size() == 0) allSink = false;
        else {
            for (int i=0; i<forwardStations.size();i++) {
                Object thisStationKey = forwardStations.get(i);
                if (!station_def.getStationType(thisStationKey).equals(STATION_TYPE_SINK)) allSink=false;
            }
        }
            ...
            for (int i=0; i<forwardStations.size(); i++) {
                ...
                if (!alreadyVisited.contains(thisForwardStation)) {
                    ...
                    if (!station_def.getStationType(thisForwardStation).equals(STATION_TYPE_SINK)) {
                        ...
                        else  temp = this.exploreForAllForwardStationAreSink(classKey,thisForwardStation,alreadyVisited);
                    }
    ...
    */
    ...
    private void checkForMoreThanOneSinkWarning () {
        ...
        if (nSink > 1) warnings[MORE_THAN_ONE_SINK_WARNING] = true;
    }
    ...
    private void checkForNoBacwardLinkWarning() {
        ...
        for (int i=0;i<stations.size(); i++) {
            Object thisStationKey = stations.get(i);
            if ( (!station_def.getStationType(thisStationKey).equals(STATION_TYPE_SOURCE)) && (!station_def.getStationType(thisStationKey).equals(STATION_TYPE_SINK)) ) {
    ...
    private void checkForForkWithoutJoinWarnings() {
        ...
        if ((fork) && (!join)) warnings[FORK_WITHOUT_JOIN_WARNING] = true;
    }
    ...
    private void checkForJoinWithoutForkErrors() {
        ...
        if ((!fork) && (join)) errors[JOIN_WITHOUT_FORK_ERROR] = true;
    }
    ...
    private boolean isTypeUniformServiceStrategy (Object stationKey) {
        ...
        if (!classes.isEmpty()) {
            ...
            for (int j=0;j<classes.size();j++) {
                ...
                if (service instanceof LDStrategy) ldFound = 1;
                else if (service instanceof Distribution) distrFound = 1;
            }
            if ((ldFound * distrFound) == 1) value = false;
        }
        return value;
    }
     ...
     private boolean mustHaveSameExponentialServiceTimes(Object station) {
         ...
         if ( (isAllFCFSQueueingStrategy(station)) &&
              (isTypeUniformServiceStrategy(station))   &&
