...
public class JobAnimation implements Animation{
    ...
    public JobAnimation(double speed, JobPath[] jps, Rectangle bounds, IconsToolkit toolkit){
        ...
        currentPath = (int)(Math.random()*(double)(jobPaths.length));
    }
    ...
    public void paint(Graphics g, ImageObserver io) {
        if(repaint)g.drawImage(icon, bounds.x, bounds.y, io);
    }
    ...
    public void setBounds(Rectangle r) {
        if(bounds.width!=r.width||bounds.height!=r.height)toolkit.getJobIcon(r);
        bounds=r;
    }
    ...
    public JobContainer getNextElement(){
        if(jobPaths[currentPath].isLast())currentPath = (int)(Math.random()*(double)(jobPaths.length));
