...
public class QueueNetAnimation extends JPanel implements Animation{
    ...
    public void refresh() {
        for(int i=0, j=0, k=0; i<edges.size() || j<stations.size() || k<jobs.size(); i++, j++, k++){
            if(i<edges.size())((Animation)edges.get(i)).refresh();
            if(j<stations.size())((Animation)stations.get(j)).refresh();
            if(k<jobs.size())((Animation)jobs.get(k)).refresh();
        }
        if(getGraphics()!=null)update(this.getGraphics());
    }
    ...
    public void paint(Graphics g, ImageObserver io) {
        if(newFrame == null) newFrame = new BufferedImage(getSize().width, getSize().height, BufferedImage.TYPE_4BYTE_ABGR);
        g.drawImage(newFrame, 0, 0, io);
        Graphics f = newFrame.getGraphics();
        if(bgImage==null) buildBGImage();
        f.drawImage(bgImage, 0, 0, io);
        //first paint all of the edges, then all of the stations
        for(int i=0; i<edges.size(); i++)((EdgeAnimation)edges.get(i)).paint(f, io);
        for(int i=0; i<jobs.size(); i++)((JobAnimation)jobs.get(i)).paint(f, io);
        for(int i=0; i<stations.size(); i++)((StationAnimation)stations.get(i)).paint(f, io);
    }
    ...
    public JobContainer[] getSuccessors(JobContainer precedant){
        ...
        if(obj instanceof StationAnimation) return new JobContainer[]{((StationAnimation)obj)};
    ...
    private Vector addElementToPath(Vector coveredPath, StationAnimation pathEnd){
        ...
        if(lastElement.equals(pathEnd)){
            ...
            if(coveredPath.size()>1)return coveredPath;
        }
        //Test if I'm on a cycle...
        if(coveredPath.indexOf(lastElement)!=coveredPath.size()-1) return null;
        ...
        if(obj instanceof Vector){
            ...
            if(v.size()==1){
                ...
                for(int i=0; i<v.size(); i++){
                    ...
                    if(i!=0) coveredPath = new Vector(coveredPath);
