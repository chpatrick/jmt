    ...
    protected static void setUserClasses(Element root, JMODELModel model) {
        ...
        for (int i=0; i<classes.getLength(); i++) {
            ...
            if (names.containsKey(name))
                ...
                System.out.println("Warning - Found color info for class '"+
                        name +
                        "' which is not present into current model.");
        }
    }
        ...
        if (temp.getLength() != 0) {
            ...
            if (Boolean.valueOf(enabled).booleanValue()) {
                ...
                try {
                    ...
                    ParametricAnalysisDefinition pad = (ParametricAnalysisDefinition)Class.forName(classPath).getConstructor(paramClasses).newInstance(params);
