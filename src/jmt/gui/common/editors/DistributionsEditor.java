...
public class DistributionsEditor extends JMTDialog {
	...
	/**
	 * Listener for choser ComboBox to instantiate a new distributions data
		...
		if (current != null) {
			...
			if (current instanceof Burst) {
				...
				if (current.hasC() || current.hasMean()) {
					...
					SpringUtilities.makeCompactGrid(mean_c_panel, rows, 2, // rows,
																			// cols
							6, 6, // initX, initY
							6, 6); // xPad, yPad
				} else {
	...
	protected class BurstRenderer {
		...
		protected void addDistribution(String name, int key,
			...
			// don't finish row because also the label for the distribution has
			// to be added
