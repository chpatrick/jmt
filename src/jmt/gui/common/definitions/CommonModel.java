...
// --- Blocking Region Definition --- Bertoli Marco ----------------------------------------------
    ...
    protected boolean canStationTypeBeBlocked(String stationType) {
        return !stationType.equals(CommonModel.STATION_TYPE_SOURCE) && !stationType.equals(CommonModel.STATION_TYPE_SINK) &&
               !stationType.equals(CommonModel.STATION_TYPE_FORK) && !stationType.equals(CommonModel.STATION_TYPE_JOIN) &&
               !stationType.equals(CommonModel.STATION_TYPE_TERMINAL);
    }
    ...
    protected void addBlockingDetailsForClass(Object classKey) {
        ...
        for(int i=0; i<blockingKeys.size(); i++){
            blockingDetailsBDM.put(classKey, blockingKeys.get(i), new BlockingClassData(
                    Defaults.getAsInteger("blockingMaxJobsPerClass"),
    ...
    protected void addBlockingDetails(Object blockingRegionKey) {
        Vector classKeys = getClassKeys();
        for(int i=0; i<classKeys.size(); i++){
            blockingDetailsBDM.put(classKeys.get(i), blockingRegionKey, new BlockingClassData(
                    Defaults.getAsInteger("blockingMaxJobsPerClass"),
                    Defaults.getAsBoolean("blockingDropPerClass")
            ));
        }
...
// --- end Blocking Region Definition ------------------------------------------------------------
    ...
    protected class ClassData{
        ...
        public ClassData(String name, int type, int priority,
                         Integer population, Object distribution){
            this.name = name;
            this.type = type;
    ...
    protected class StationClassData{
        ...
        public StationClassData(String queueStrategy,
                                Object serviceDistribution,
    ...
    protected class MeasureData{
        ...
        public MeasureData(Object station,
                           Object userClass,
                           String measureType,
                           Double alpha,
                           Double precision){
