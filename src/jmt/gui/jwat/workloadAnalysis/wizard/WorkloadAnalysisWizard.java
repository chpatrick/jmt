 ...
 */
package jmt.gui.jwat.workloadAnalysis.wizard;
...
public class WorkloadAnalysisWizard extends JWatWizard {
	...
	private void initGUI(){
		...
		CodeSource s = c.getProtectionDomain().getCodeSource();
		URL url = s.getLocation();
		String st = url.toString().substring(0, url.toString().lastIndexOf("/"));
		String st1 = st.substring(st.indexOf(':')+2, st.length());
		String st2 = st1.replaceFirst("%20", " ").replaceFirst("%20", " ");
		File f = new File(st2 + "/jwatFormats/Apache log file format.jwatformat");
	...
	private JMenuBar getWorkloadMenuBar() {
		if(workloadMenubar == null){
			...
			JMenuItem[][] menuItems = {	{new JMenuItem(FILE_NEW),new JMenuItem(FILE_SAVE){{setEnabled(false);}},new JMenuItem(FILE_OPEN){{setEnabled(false);}},null, new JMenuItem(EXIT_ACTION)},
					...
					{new JMenuItem(HELP_SHOWHELP){{setEnabled(true);}},null,new JMenuItem(HELP_CREDITS)}};
