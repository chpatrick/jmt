...
public class BDMapImpl implements BDMap{
    ...
    public Set keySet(int coordName){
        if(coordName == X) return getSetFromArray(xKeys);
        if(coordName == Y) return getSetFromArray(yKeys);
        else return null;
    }
    ...
    public boolean containsKey(Object key, int coordName){
        if(coordName == X)return -1!=getIndex(xKeys, key);
        if(coordName == Y) return -1!=getIndex(yKeys, key);
        else return false;
    }
    ...
    public void put(Object key, int coordName, Map newMap){
        ...
        if(coordName == X){
            ...
            if(keyIndex!=-1) replaceColumn(keyIndex, objects);
            else{
            ...
            if(keyIndex!=-1) replaceRow(keyIndex, objects);
            else{
