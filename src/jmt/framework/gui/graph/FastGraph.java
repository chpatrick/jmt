...
public class FastGraph extends JPanel {
    ...
    public void paint(Graphics g) {
        ...
        for (int i=0; i<values.size() - 1; i++) {
            ...
            if (value > 0 && !Double.isInfinite(value))
                g.drawLine(getX(i*xunit), getY(value),
                        getX((i+1)*xunit), getY(((MeasureValue)values.get(i+1)).getUpperBound()));
            // lower bound
            value = ((MeasureValue)values.get(i)).getLowerBound();
            if (value > 0 && !Double.isInfinite(value))
                g.drawLine(getX(i*xunit), getY(value),
                        getX((i+1)*xunit), getY(((MeasureValue)values.get(i+1)).getLowerBound()));
        }
        ...
        for (int i=0; i<values.size() - 1; i++) {
            g.drawLine(getX(i*xunit), getY(((MeasureValue)values.get(i)).getMeanValue()),
                    getX((i+1)*xunit), getY(((MeasureValue)values.get(i+1)).getMeanValue()));
        }
        g.fillOval(getX(((values.size()-1))*xunit),
                getY(((MeasureValue)values.get(values.size()-1)).getMeanValue()),
                2,1);
    }
